...
import { createSign, createPrivateKey, randomUUID, createHash } from 'crypto';
...
${attrXml}
   ...
   *
   * LIMITATION: This implementation does not perform full exclusive C14N as
   * specified in http://www.w3.org/2001/10/xml-exc-c14n# — specifically, it
   * does NOT render namespace declarations (xmlns:* attributes) according to
   * the exc-c14n visibility rules, which require propagating only the namespaces
   * that are actually used by each element and its descendants rather than all
   * namespaces declared on ancestor elements.  Because all namespace
   * declarations are inlined directly on the elements that use them in the SAML
   * XML generated by this service, the output is functionally equivalent for
   * our use case, but it will differ from a standards-compliant exc-c14n
   * implementation when verifying against a third-party SAML library that
   * applies the full algorithm.
   */
